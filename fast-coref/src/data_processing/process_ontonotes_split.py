...
def minimize_partition(split, args):
    ...
    with open(input_path, "r") as input_file:
        pattern = re.compile(r"#begin document \((.*)\); part (\d+)___(\d+)")
        for line in input_file.readlines():
            begin_document_match = re.match(pattern, line)
            if begin_document_match:
                doc_key = "{}_{}_{}".format(begin_document_match.group(1),
                                  begin_document_match.group(2),
                                  begin_document_match.group(3))
